...
import { getOrderDetail } from '../api/orderApi'; // Corrected function name
...
const OrderHistory = () => {
    ...
    useEffect(() => {
        ...
        const fetchOrders = async () => {
            try {
                setLoading(true);
                const response = await getOrderDetail(13); // Fetch details for order ID 13
                console.log("API order details data:", response.data); // debug
                if (response.data && response.data.totalAmount) {
                    setOrders([response.data]); // Set orders with the fetched order details
                } else {
                    console.error("Expected order details but received:", response.data);
                }
            } catch (err) {
    ...
    return (
        <div className="order-history-page">
            <div className="order-history-header">
                <h1><FaShoppingBag /> Lịch sử đơn hàng</h1>
            ...
            {loading ? (
                ...
                <div className="orders-grid">
                    {orders.map((order) => {
                        const totalAmount = order.totalAmount || 0; // Ensure totalAmount is a valid number
                        return (
